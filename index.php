...
/* ── xwt.csv(약장 배치도) 기준 초기값 ── 열 → 행 → [약재들, 위→아래 순] ──
   틀린 칸은 서랍 탭 → 수정, 또는 시트 붙여넣기로 덮어쓰기 하세요. */
const GRID = {
 "1":{
  1:["토사자","보골지"],2:["파극","행인"],3:["현호색","필발"],4:["패모","황정"],
  5:["황금","황련"],6:["홍화자","패창초"],7:["포항","해동피"],8:["해백(총백)","산자고"],
  9:["호장근","핀람근"],10:["현삼","태자삼","해풍동"],11:["황백","토복령"],12:["홍화","합한피"],
  13:["현지초","택란","오적골"],14:["삼백초","천초근","화피"],15:["해초","생강피","포도근"],
  16:["패란","측백엽","투골초"],17:["반지련","편수","희령"],18:["자석영","한련초"]
 },
 "2":{
  1:["인삼","숙지황"],2:["황기","두충"],3:["후박","대추"],
  4:["오미자","여정자","오매","우방자"],5:["지골피","옥죽","오수유","연자육"],
  6:["지실","지모","지각","적하수오"],7:["전호","진교","차전자","적작약"],
  8:["청피","천련자","초오","천오"],9:["저령","창이자","자연등(중웅자)","정력자"],
  10:["천화분","적복령","지구자","청상자"],11:["마자인(화마인)","자완","초과","전칠"],
  12:["천문동","정향","지부자","천(화)초(산초)"],13:["초두구","자석","전과루","적석지"],
  14:["연자심","유근백피","저근백피"],15:["자초","자화지"],16:["죽여","죽엽"],
  17:["인동등","자초","천축"],18:["천마","조각자"]
 },
 "3":{
  1:["감초","계피(육계)","길경"],2:["구기자","복령","산수유","반하"],
  3:["목향","맥문동","신곡","맥아"],4:["시호","목단피","산사","창출"],
  5:["진피","우슬","산약","방풍"],6:["원지","산조인","(산)치자","향부자"],
  7:["승마","속단","석창포","택사"],8:["석결명","석고","몰약","유향"],
  9:["오배자","황석","시채","용치"],10:["산두근","석남등","사상자","삼칠근"],
  11:["야교등","육두구","양기석","용근까마중"],12:["오약","율금","왕불유행","양강"],
  13:["익지인","우담자","야명초","우절"],14:["지유","연수","사원자"],
  15:["용담초","신근초"],16:["연고","선학초"],17:["오가피","어성초"],18:["육이인","위령선"]
 },
 "4":{
  1:["백출","백작약","천궁","사인"],2:["파지","자감초(밀왕기)","검정콩"],3:["당귀","홍삼(홍삼미)","녹용"]
 },
 "5":{
  9:["복신","백하수오","복분자","백전"],10:["부자","백지","백합","봉(아출)"],
  11:["백모근","백부","비자","불수"],12:["부소맥","선모","쇄양","백령"],
  13:["소회향","백선피","사군자","밀봉화"],14:["소목","소자","백굴채","상심자"],
  15:["신이화","선복화"],16:["상백피","상지"],17:["조구등","상기생"],18:["세신","송절"]
 },
 "6":{
  8:["단삼","대황","도인","당삼"],9:["대극","대계","대회향","담두시"],
  10:["대자석(자석)","동규자","대복피","동과인"],11:["망초","만형자","목통","목과(모과)"],
  12:["생지황","백자인","복령피","백계자"],13:["사간","삼릉","배","백질려"],
  14:["백편두","백두구","비해"],15:["마치현","등심"],16:["백두웅","백미"],
  17:["비파엽","부평초"],18:["번사엽","목적"]
 },
 "7":{
  7:["계지","갈근"],8:["강활","독활"],9:["건강","결명자","나복자"],10:["고본","남성","검실"],
  11:["골쇄보","곡아","귤홍피"],12:["괴화","과루인","금앵자","가자"],13:["국화","고삼","노근","노희"],
  14:["감국","구판","대풍자","과루피"],15:["금은화","금전초","구절초"],16:["계혈등","국화","구맥"],
  17:["곤포","관공화","강황"],18:["구척","곽향","대청"]
 },
 "8":{
  2:["유근피","계지 오약"],3:["용담초","화고초"],4:["박하","포공영"],
  5:["옥미수","상엽"],6:["양쑥(애엽)","인진쑥"],7:["익모초","음양각"],8:["소엽","형개"]
 },
 "9":{
  1:["행인가루"],2:["도인"],3:["목적"],4:["대(소)계"],5:["고이"],6:["곡정"],
  7:["동과피"],8:["마두령"],9:["권백"],10:["천심련"]
 },
 "R":{
  1:["앵도육"],2:["화마인(마자인)"],3:["잣"],4:["건율"],5:["호도"],6:["표고"],7:["은행"],
  8:["연자심"],9:["용안육"],10:["백강정"],11:["사과"],12:["당근"],13:["생강"],14:["호박"],15:["비트"]
 }
};

/* 자주 쓰는 약재 일부만 한자·분류·메모 보강 (위치는 위 GRID 기준) */
const ENRICH = {
 "인삼":{hanja:"人蔘",cat:"보기",note:"대보원기(大補元氣). 기허·피로에."},
 "황기":{hanja:"黃芪",cat:"보기"}, "백출":{hanja:"白朮",cat:"보기"},
 "당귀":{hanja:"當歸",cat:"보혈"}, "숙지황":{hanja:"熟地黃",cat:"보혈"},
 "백작약":{hanja:"白芍藥",cat:"보혈"}, "천궁":{hanja:"川芎",cat:"활혈"},
 "맥문동":{hanja:"麥門冬",cat:"보음"}, "구기자":{hanja:"枸杞子",cat:"보음"},
 "산수유":{hanja:"山茱萸",cat:"보음"}, "오미자":{hanja:"五味子",cat:"보음"},
 "갈근":{hanja:"葛根",cat:"해표"}, "생강":{hanja:"生薑",cat:"해표"},
 "계지":{hanja:"桂枝",cat:"해표"}, "시호":{hanja:"柴胡",cat:"해표"},
 "감초":{hanja:"甘草",cat:"기타",note:"조화제약(調和諸藥)."}, "대추":{hanja:"大棗",cat:"기타"},
 "복령":{hanja:"茯苓",cat:"기타"}, "반하":{hanja:"半夏",cat:"화담"},
 "진피":{hanja:"陳皮",cat:"이기"}, "후박":{hanja:"厚朴",cat:"이기"},
 "길경":{hanja:"桔梗",cat:"화담"}, "황금":{hanja:"黃芩",cat:"청열"},
 "황련":{hanja:"黃連",cat:"청열"}, "석고":{hanja:"石膏",cat:"청열"}, "황백":{hanja:"黃柏",cat:"청열"},
};

/* 초기 데이터 — 위 GRID(약장 배치도) 기준.
   창고 위치는 비워 둡니다. 실제 배치는 데이터 메뉴의 「약재창고 가져오기」로 넣으세요. */
function buildSeed(){
  const out=[];
  for(const yeol in GRID){
    const cols=GRID[yeol];
    for(const haeng in cols){
      cols[haeng].forEach(name=>{
        if(!name || name==="-") return;
        const e=ENRICH[name]||{};
        out.push({ name, hanja:e.hanja||"", cat:e.cat||"기타",
          qty:0, full:50, min:0, unit:"돈",
          yeol, haeng:String(haeng),                // 약장(검색) 위치
          zone:"", wyeol:"", whaeng:"",             // 창고 위치 — 가져오기로 채움
          note:e.note||"" });
      });
    }
  }
...
/* ── 초기 데이터 넣기 — DB가 비어 있을 때만 ── */
async function seedPrompt(){
  const seed=buildSeed();
  if(!confirm(`데이터베이스가 비어 있습니다.\n약장 배치도 기준 ${seed.length}종을 넣을까요?\n\n· 수량은 모두 0으로 시작합니다\n· 창고 위치는 비어 있습니다 (데이터 → 약재창고 가져오기로 채우세요)`)) return;
  toast(`${seed.length}종 넣는 중…`);
  try{
    const r=await api("seed",{items:seed});
    state=r.data; render(); toast(`${r.added}종을 넣었습니다`);
  }catch(err){
    toast(err.message);
  }
}
